...
public class RequestSavedAnnounce implements Message {
    @Override
    public void deserialize(JSONObject data, ChannelHandlerContext channelHandlerContext, DatabaseService databaseService) {
        ...
        baseQuery.removeIf(announce -> !announce.getFavorites().contains(channelHandlerContext.channel().attr(PHONE).get()));
