...
public class RequestBaseAnnounce implements Message {
    ...
    public void deserialize(JSONObject data, ChannelHandlerContext channelHandlerContext, DatabaseService databaseService) {
        ...
        if (data.has("number"))
            channelHandlerContext.channel().attr(PHONE).set(data.getString("number"));
        ...
        currentQuery.forEach(announce -> {
            ...
            System.err.println("Phone = " + channelHandlerContext.channel().attr(PHONE).get());
        ...
        System.err.println("Send -> " + data.toString());
